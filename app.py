...
★문체: 반드시 한국 대기업식 '개조식 요약문'으로 작성할 것. (예: "~임", "~로 판단됨", "파급력 큼", "고성능 확보" 등 명사형 또는 명사조사로 문장 종결. "~습니다", "~해요" 사용 금지)"""
    ...
    if prompt := st.chat_input("기사 URL을 입력하거나 질문을 남겨주세요."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("책임 연구원이 기술 분석을 수행 중입니다..."):
                try:
                    # URL 감지 및 본문 추출
                    article_text = ""
                    if "http" in prompt:
...
# 하단 푸터
st.markdown("<div style='text-align: center; color: #bbb; font-size: 11px; padding-top: 50px;'>© 2024 Solid-State Battery Technical Analysis Chatbot. All rights reserved.</div>", unsafe_allow_html=True)
