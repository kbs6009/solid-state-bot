...
# 7. 모델 생성 (★구글 검색 도구 활성화)
model = genai.GenerativeModel(
    model_name="gemini-2.5-flash", 
    tools=[{"google_search": {}}], # 구글 검색 기능 연결
    ...
    with st.chat_message("assistant"):
        with st.spinner("구글 검색과 기사 분석을 동시에 진행 중입니다..."):
            try:
                ...
                # 기사 내용 + 검색 요청 메시지 구성
                if article_text:
                    final_input = f"다음 기사 내용을 분석하고, 구글 검색을 통해 관련 인물 이력과 기술적 배경, 참고 문헌을 추가해서 리포트를 작성해줘:\n\n{article_text}"
                else:
                    final_input = f"구글 검색을 활용하여 다음 질문에 대해 전문적인 분석 리포트를 작성해줘: {prompt}"
